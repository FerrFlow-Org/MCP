...
| `FERRLABS_MCP_ALLOW_TOKEN_REVEAL` | Set to `1` to let `create_token` mint a token and return its secret. Off by default: the secret is shown once, so the tool has to put it in its response, and tool results are persisted to the client transcript. | —                                                                         |
| `FERRLABS_MCP_MAX_TOOL_BYTES`     | Ceiling on a single tool response, in bytes. Beyond it the payload is cut and explicitly marked as incomplete.                                                                                                     | `100000`                                                                  |
